...
class DispatchingCenterListPage extends StatefulWidget {
  ...
  @override
  State<DispatchingCenterListPage> createState() => _DispatchingCenterListPageState();
}
        ...
        child: SafeArea(
          ...
          child: Stack(
            children: [
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        _RoundIconButton(icon: Icons.emoji_events_outlined, onTap: () {}),
                      ],
                    ),
                  ),
                  Expanded(
                        ...
                        ...List.generate(_rooms.length - 3, (index) {
                          ...
                          return Padding(
                            ...
                            child: _DispatchRoomCard(
                              ...
                              onTap: () => setState(() => _selectedIndex = index + 3),
                            ),
                          );
                        }),
                      ],
              ...
              const Align(alignment: Alignment.bottomCenter, child: _BottomDockPreview()),
            ],
          ),
        ),
      ),
...
class _HeaderTab extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Text(label, style: TextStyle(fontSize: active ? 24 : 17, fontWeight: active ? FontWeight.w900 : FontWeight.w700, color: active ? const Color(0xFF111111) : const Color(0xFF8B8B8B)));
  }
}
...
class _RoundIconButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Material(color: Colors.white.withValues(alpha: 0.90), shape: const CircleBorder(), child: InkWell(customBorder: const CircleBorder(), onTap: onTap, child: SizedBox(width: 40, height: 40, child: Icon(icon, size: 22, color: const Color(0xFF222222)))));
  }
}
...
class _TopHeroGrid extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Row(children: [for (int i = 0; i < rooms.length; i++) ...[Expanded(child: _TopHeroCard(room: rooms[i])), if (i != rooms.length - 1) const SizedBox(width: 10)]]);
  }
}
      ...
      child: Padding(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: const Color(0xFFBDA8FF).withValues(alpha: 0.22), blurRadius: 26, offset: const Offset(0, 12))],
          ),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: const [
              _DockItem(icon: Icons.home_outlined, label: '首页', active: false),
              _DockItem(icon: Icons.star_border_rounded, label: '圈子', active: false),
              _DockCenterButton(),
              _DockItem(icon: Icons.chat_bubble_outline_rounded, label: '消息', active: false),
              _DockItem(icon: Icons.person_outline_rounded, label: '我的', active: false),
            ],
          ),
        ),
      ),
...
class _DockItem extends StatelessWidget {
  const _DockItem({required this.icon, required this.label, required this.active});
  final IconData icon;
  final String label;
  final bool active;
  ...
  @override
  Widget build(BuildContext context) {
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 22, color: active ? const Color(0xFF7A5CFF) : const Color(0xFFB8B8B8)), const SizedBox(height: 3), Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: active ? const Color(0xFF7A5CFF) : const Color(0xFFB8B8B8)))]);
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: const LinearGradient(colors: [Color(0xFFFFD7B5), Color(0xFFFF6FB3), Color(0xFF8A6CFF)]),
        boxShadow: [BoxShadow(color: const Color(0xFFFF7AB6).withValues(alpha: 0.28), blurRadius: 18, offset: const Offset(0, 10))],
      ),
